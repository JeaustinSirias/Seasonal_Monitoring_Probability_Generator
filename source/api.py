...
class App():
    ...
    def __init__(self, master):
        ...
        self.label0 = tk.Label(
            frame, 
            text = 'Climatological period setup'
        )
        self.label_analogSettings=tk.Label(
            frame, 
            text='Analog years settings'
        )
        self.labelz = tk.Label(
            frame, 
            text='Season to monitor'
        )
        ...
        self.label5 = tk.Label(
            frame, 
            text='Number of analog years to compute:'
        )
        self.label7 = tk.Label(
            frame, 
            text='Analyisis preferences'
        )
        ...
        self.HELP = tk.Button(
            frame, 
            text='About', 
            command=lambda: self.Help(master)
        )                                                                                                                                                                              
        ...
        self.FORECAST = tk.Radiobutton(
            frame, 
            text='Forecast', 
            variable=self.V7, 
            value=True
        )
        self.ANALYSIS = tk.Radiobutton(
            frame, 
            text='Observed data', 
            variable=self.V7, 
            value=False
        )
        self.SAVE = tk.Checkbutton(
            frame, 
            text='Save reports', 
            variable=self.V8
        )
        self.DISPLAY = tk.Checkbutton(
            frame, 
            text='Display reports', 
            variable=self.V9
        )
        self.SCENARIO = tk.Checkbutton(
            frame, 
            text='Include scenarios', 
            variable=self.V6
        )
        self.RUN = tk.Button(
            frame, 
        ...
        # MENUS
        self.init_clim = Combobox(
            frame, 
            textvariable=self.V1, 
            values=[None]
        )
        self.end_clim = Combobox(
            frame, 
            textvariable=self.V2, 
            values=[None]
        )
        self.start_dekad = Combobox(
            frame, 
            textvariable=self.V3, 
            values=tuple(self.dekad_lst)
        )
        self.end_dekad = Combobox(
            frame, 
            textvariable=self.V4, 
            values=tuple(self.dekad_lst)
        )
        self.analog_menu  = Combobox(
            frame, 
    ...
    def Browse(self):
        ...
        if not file is None:
            try:
                fst_yr, lst_yr, IDs, raw_data, scenario = read(file)
            except ValueError:
        ...
        self.dataset = fst_yr, lst_yr, IDs, raw_data, scenario
    ...
    def Run(self, fst_cm, lst_cm, fst_dk, lst_dk, analogs_num):
        ...
        fst_yr, lst_yr, IDs, raw_data, scenario = self.dataset
        fst_cm = int(fst_cm)
        lst_cm = int(lst_cm)
        ...
        if scenario == [] and self.V6.get():
            tk.messagebox.showerror('Error', 
            'There are no values to compute custom scenarios in this dataset.')
            return 
        ...
        try:
            ...
            SMPG = smpgTool(fst_yr, lst_yr, fst_cm, lst_cm, fst_dk, lst_dk, scenario, SIZE, analogs_num, 
            ...
            ca_stats, ca_pctl = SMPG.climatological_accumulation(Dict)
            ...
            # Scenarios
            if self.V6.get(): #if user is asking for scenarios
            ...
            clm_sts, analogs, an_scns, clim_scns, dirpath=Dir) 
